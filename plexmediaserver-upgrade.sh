...
if [[ $latestversion > $currentversion ]]; then
	...
	if [[ $(dpkg -s plexmediaserver | awk -F'[- ]' '/^Version:/ { print $2 }') = $latestversion ]]; then
	  printf -- "plexmediaserver upgraded successfully from version %s to %s... \n" $currentversion $latestversion;
	  printf -- "%(%Y-%m-%d %H:%M:%S)T [SUCCESS] plexmediaserver upgraded to %s... \n" $(date +%s) $latestversion | tee -a update.log >/dev/null;
	  ...
	  printf -- "%(%Y-%m-%d %H:%M:%S)T [ERROR] plexmediaserver %s upgrade failed... \n" $(date +%s) $latestversion | tee -a update.log >/dev/null;
	fi
else
